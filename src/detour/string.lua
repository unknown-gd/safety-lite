...
_G.string.format = Detour.attach(string.format, function(hk, format, ...)
	...
	if did_shadow then
		Log( 3, "Detoured string.format(%q, ...)", format )
	end
...
_G.string.rep = Detour.attach(string.rep, function(hk, str, reps, sep)
	...
	if ( len( str ) * reps ) + ( d_isstring(sep) and #sep or 0 ) * reps > MAX_REP then
		return Log(2, "Someone tried to string.rep with a fucking massive return string!")
	end
